...
Every mark is a **CSS mask**: `css/marks.css` (generated by the build from the
SVGs, so the ratios cannot drift from the art) points `mask-image` at the file
and paints it with `background-color: currentColor`. They are single-colour
silhouettes, so this recolours exactly as `fill` did, and the five files are
fetched once for the whole site instead of being carried in every page.

The route here: the motif was inlined twenty-two times on the catalogue page,
40 KB each, which made the front page 1.13 MB. An inline sprite took that to
188 KB, but the sprite was still 66 KB gzipped — ninety per cent of every page,
repeated across all twenty-five because HTML is not cached between them. Masks
take a page to **28 KB raw, 6 KB gzipped**, and the whole `shop/` directory
from 3.4 MB to 240 KB.

A mask has no intrinsic size, which an inline SVG did. Anything sizing a mark
needs a length somewhere: a percentage width inside an `auto` grid column
resolves against a column that is waiting on the mark, and both collapse to
zero.
